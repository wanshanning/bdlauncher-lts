...
public:
    ...
    MoneyCommand(CommandOrigin const &origin, CommandOutput &output) noexcept : CustomCommandContext(origin, output) {}

    void query(mandatory<Query> cmd);
    void pay(mandatory<Pay> cmd, mandatory<std::string> target, mandatory<int> count);
